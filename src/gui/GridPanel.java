        ...
        PseudoInfiniteViewport.Pannable
{
     ...
     * empty.
     * 
     * @param factor the difficulty factor of the game
     */
    public GridPanel(DisplayMap map, ResourceBundle res, double factor)
    {
        displayMap = map;
        resources = res;
    ...
    /**
     * Zooms in the display by increasing the cell size relative to the difficulty of the game
     */
    public void zoomIn()
    {
    	cellSize *= 1.34;
    ...
    /**
     * Pans the display back to the origin, so that the User is in the middle of the viewpoint
     */
    public void recenter(Location loc)
    {
        originRow = loc.getRow() - 3;
        originCol = loc.getCol() - 3;
        repaint();
        ...
        if (vp != null)
        {
            if (!isPannableUnbounded()
                ...
                vp.setViewPosition(pointForLocation(new Location(grid.getUser().getLocation().getRow() - 3,grid.getUser().getLocation().getCol() - 3),true));
            else
                showPanTip();
        }
    }
    ...
    public Point pointForLocation(Location loc, boolean b)
    {
        return new Point(colToXCoord(loc.getCol()) + cellSize / 2 + 55,
                rowToYCoord(loc.getRow()) + cellSize / 2 + 55);
    }
    ...
    /**
     * displays the information of a specified actor at loc
